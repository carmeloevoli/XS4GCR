...
void gamma_param_nd(double Tp, PARAMSET *params) {
    ...
    if ((Tp > 0.487) && (Tp < 512000.1)) {
        ...
        params->a[1] = -1.2592e-5 + 1.4439e-5 * exp(-0.29360 * (y + 3.4)) + 5.9363e-5 / (y + 4.1485) +
                       y * (2.2640e-6 - 3.3723e-7 * y);
        ...
        z = y + 4.7171;
        params->a[5] = 9.0466e-7 + 1.4539e-6 * log10(0.015204 * (y + 3.4)) + 0.00013253 / (z * z) +
                       y * (-4.1228e-7 + 2.2036e-7 * y);
...
void gamma_param_diff(double Tp, PARAMSET *params) {
    ...
    if ((Tp > 1.94) && (Tp < 512000.1)) {
        ...
        params->b[6] = 1.7843 + y * (0.91914 + y * (0.050118 + y * (0.038096 + y * (-0.027334 + y * (-0.0035556 +
                                                                                                     0.0025742 * y)))));
        z1 = y + 1.8441;
    ...
    if ((Tp < 0.488) || (Tp > 1.95)) {
        ...
        if (x < A) {
            ...
            if ((pt_params->b[0][1] != 0) && (pt_params->b[0][2] != 0)) {
                pt_params->b[0][0] = 2 * sigma_incl / (pt_params->b[0][1] * sqrt(M_PI * pt_params->b[0][2]) *
                                                       (erf(pt_params->b[0][1] / sqrt(pt_params->b[0][2])) + 1) +
                                                       pt_params->b[0][2] *
                                                       exp(-pt_params->b[0][1] * pt_params->b[0][1] /
                                                           pt_params->b[0][2]));
            } else {
                pt_params->b[0][0] = 0;
            }
    ...
    if ((Tp < 0.69) || (Tp > 2.76)) {
        ...
        if (x < 0.5) {
            ...
            if ((pt_params->c[0][1] != 0) && (pt_params->c[0][2] != 0)) {
                pt_params->c[0][0] = 2 * sigma_incl / (pt_params->c[0][1] * sqrt(M_PI * pt_params->c[0][2]) *
                                                       (erf(pt_params->c[0][1] / sqrt(pt_params->c[0][2])) + 1) +
                                                       pt_params->c[0][2] *
                                                       exp(-pt_params->c[0][1] * pt_params->c[0][1] /
                                                           pt_params->c[0][2]));
            } else {
                pt_params->c[0][0] = 0;
            }
