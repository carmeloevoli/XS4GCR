...
double Kamae2006_Secondary_Leptons::get_cparamlib_sigma(const PID& lepton_,
                                                        const double& T_proton_GeV_,
...
double Kamae2006_Secondary_Leptons::get(const PID& projectile, const TARGET& target,
    ...
    if (projectile.is_H() && target.is_H()) {
        ...
        return sigma_pp * HeHe_correction *
               ((lepton.is_electron()) ? He_function(T_n, T_lepton) : 1);
    } else {
...
double HuangPohl2007_Secondary_Leptons::get(const PID& projectile, const TARGET& target,
                                            const double& T_n, const double& T_lepton) const {
...
std::vector<double> read_spectra_file(const std::string& filename, const size_t& x_size,
                                      const size_t& y_size) {
    std::vector<double> v;
    char s[20];
...
void HuangPohl2007_Secondary_Leptons::read_data_file() {
    std::vector<double> eminus_p_spectra =
        read_spectra_file(eminus_p_datafile, T_lepton_size, E_proj_size);
    std::vector<double> eminus_he_spectra =
        read_spectra_file(eminus_he_datafile, T_lepton_size, E_proj_size);
    std::vector<double> eplus_p_spectra =
        read_spectra_file(eplus_p_datafile, T_lepton_size, E_proj_size);
    std::vector<double> eplus_he_spectra =
        read_spectra_file(eplus_he_datafile, T_lepton_size, E_proj_size);
    ...
    for (size_t ix = 0; ix < T_lepton_size; ++ix) {
        for (size_t iy = 0; iy < E_proj_size; ++iy) {
            if (lepton.is_electron()) {
                dsigma_dT_H.get(ix, iy) =
                    eminus_p_spectra.at(ix * E_proj_size + iy) * prod_xsec_p.at(iy) * units;
                dsigma_dT_He.get(ix, iy) =
                    eminus_he_spectra.at(ix * E_proj_size + iy) * prod_xsec_he.at(iy) * units;
            } else if (lepton.is_positron()) {
                dsigma_dT_H.get(ix, iy) =
                    eplus_p_spectra.at(ix * E_proj_size + iy) * prod_xsec_p.at(iy) * units;
                dsigma_dT_He.get(ix, iy) =
                    eplus_he_spectra.at(ix * E_proj_size + iy) * prod_xsec_he.at(iy) * units;
            }
        }
    }
    return;
