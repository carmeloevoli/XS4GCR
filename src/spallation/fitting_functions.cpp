...
double SigmaFunction::sigma_le(const double& T) const {
	...
	le /= pow2(pow2(T) - pow2(M)) + pow2(T * M);
	return le;
}

double SigmaFunction::sigma_he(const double& T) const {
	const double T_h = 2. * GeV;
...
void FittingFunctions::read_data_file() {
	...
	double E_th, sigma_0, Gamma, M, sigma_1, xi, Delta, sigma_ratio;
	while (infile >> Z_p >> A_p >> Z_c >> A_c >> E_th >> sigma_0 >> Gamma >> M >> sigma_1 >> xi
			>> Delta >> sigma_ratio) {
		PID child(Z_c, A_c);
		PID parent(Z_p, A_p);
		SigmaFunction function { E_th * MeV, sigma_0 * mbarn, Gamma * MeV, M * MeV, sigma_1 * mbarn, xi,
				Delta };
