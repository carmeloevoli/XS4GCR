...
double Secondary_Antiprotons::get_total_inelastic(const TARGET& target, const double& T_ap) {
    double value = 0;
    if (target.is_H()) {
        double E_GeV = (T_ap + cgs::proton_mass_c2) / cgs::GeV;
        double R_GV = std::sqrt(E_GeV * E_GeV - cgs::proton_mass_c2 * cgs::proton_mass_c2);
        ...
        value = sigma_0 *
                (1. - 0.62 * std::exp(-E_inc / 200) * std::sin(10.9 * std::pow(E_inc, -0.28)));
    }
...
double Secondary_Antiprotons::get_non_annihilating_inelastic(const TARGET& target,
                                                             const double& T_ap) {
    return get_total_inelastic(target, T_ap) - get_annihilating_inelastic(target, T_ap);
}

TanNg1983_antiprotons::TanNg1983_antiprotons() { set_model_name("TanNg1983"); }
...
DiMauro_Secondary_Antiprotons::DiMauro_Secondary_Antiprotons() { set_model_name("DiMauro"); }
...
double DiMauro_Secondary_Antiprotons::get(const PID& particle, const TARGET& target,
                                          const double& T_n, const double& T_ap) const {
...
Winkler2017_antiprotons::Winkler2017_antiprotons()
    ...
    table.read_data_file(data_filename, cgs::GeV, cgs::mbarn / cgs::GeV);
}
...
Feng2016_antiprotons::Feng2016_antiprotons(generators g)
    ...
    table.read_data_file(filename, cgs::GeV, cgs::barn / cgs::GeV);
}
